...
```sh
npm install git+ssh://git@github.com:stainless-sdks/argot-openai-node.git
```
...
async function main() {
  const params: ArgotOpenAI.CompletionCreateParams = { model: 'string', prompt: 'This is a test.' };
  const completion: ArgotOpenAI.CompletionCreateResponse = await client.completions.create(params);
}
...
async function main() {
  const completion = await client.completions
    ...
    .catch(async (err) => {
      if (err instanceof ArgotOpenAI.APIError) {
...
add the following import before your first import `from "ArgotOpenAI"`:
